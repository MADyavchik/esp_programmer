...
async def start_settings_menu():
    while True:
        ...
        index = await run_menu(menu_items, visible_lines=4, highlight_color="ivory")
...
async def change_print_quantity():
    ...
    idx = await run_menu(options, visible_lines=4, highlight_color="ivory")
...
async def change_shutdown_timeout():
    ...
    idx = await run_menu(labels, visible_lines=4, highlight_color="ivory")
    if isinstance(idx, int):
